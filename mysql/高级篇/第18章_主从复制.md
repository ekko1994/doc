...
![image-20220211133114594](images/image-20220211133114594.png)
...
![image-20220211133224805](images/image-20220211133224805.png)
...
![image-20220211133403943](images/image-20220211133403943.png)
...
![image-20220211135021427](images/image-20220211135021427.png)
...
进行主从同步的内容是二进制日志，它是一个文件，在进行`网络传输`的过程中就一定会`存在主从延迟`（比如 500 ms），这样就可能造成用户在从库上读取的数据不是最新的数据，也就是主从同步中的数据不一致性问题。
...
- 采用中间件的方法有很明显的优势，`功能强大`，`使用简单`。但因为在客户端和数据库之间增加了中间件层会有一些性能损耗，同时商业中间件也是有使用成本的。我们也可以考虑采取一些优秀的开源工具。
  ...
  ![image-20220211135730483](images/image-20220211135730483.png)
...
![image-20220211135840634](images/image-20220211135840634.png)

![image-20220211135845791](images/image-20220211135845791.png)

主备切换：

![image-20220211135859963](images/image-20220211135859963.png)
